...
const Image = ({
  ...
  return (
    <div className="relative z-0">
      <div
        className={`${
            index === 0 && images.length > 5 ? "w-[12rem] sm:w-[25rem] h-full" : "w-[12rem]"
      ...
      >
        <div className={`z-10 absolute transition duration-300 ease-in-out opacity-0 hover:opacity-40 rounded-lg bg-black  ${
              index === 0 && images.length > 5 ? "w-[23rem] sm:w-[24rem] h-full" : "w-[11rem] h-full"
            } `}></div>
        ...
        <img
          ...
          alt={image.title}
          className={`w-full ${
            index === 0 && images.length > 5 ? "sm::h-[24rem]" : " h-[11rem]"
